...
export function LocationSelectionMap({
  ...
  const { reverseGeocode, isLoading: isReverseGeocoding } = useReverseGeocoding();
  ...
  const handleMapClick = useCallback(
    async (e: maplibregl.MapMouseEvent) => {
      ...
      if (reverseResult && isMountedRef.current) {
        ...
        if (markerRef.current) {
          markerRef.current.setPopup(
            new maplibregl.Popup({ offset: 25 }).setHTML(
              `<div class="px-2 py-1 text-sm font-medium text-slate-900">${reverseResult.name}${reverseResult.country ? `, ${reverseResult.country}` : ''}</div>`
            )
          );
        }
      }
  ...
  return (
    <div className={`relative w-full h-full rounded-xl overflow-hidden ${className}`}>
      ...
      {coordinates && (
        <div className="absolute bottom-4 left-4 right-4 md:left-auto md:right-4 md:bottom-4 md:w-72 glass-strong rounded-lg p-3 shadow-xl animate-fade-in">
